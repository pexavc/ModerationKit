...
open class ScaleTexture : Handler, HandlerContextRecipient {
    ...
    var adaptive: SequenceHandler<PaddedScaleTexture.Output>?
    ...
    func AdaptivePipeline(
        ...
        width: Int,
        height: Int) -> SequenceHandler<PaddedScaleTexture.Output> {
    ...
    public func handle(input: MTLTexture, state: HandlerState) throws -> MTLTexture? {
        ...
        guard let output = context?.device.makeTexture(descriptor: outputTextureDescriptor) else {
            throw HandlerRuntimeError.genericError(self, "Cannot make texture to scale the texture")
        }
